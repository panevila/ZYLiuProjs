...
public class AverageDriver  extends Configured implements Tool {
   ...
   private static Logger THE_LOGGER = Logger.getLogger(AverageDriver.class);
...
//      THE_LOGGER.info("run(): status="+status);
   ...
   */
   public static void main(String[] args) throws Exception {
      ...
      if (args.length != 2) {
         THE_LOGGER.warn("usage P1BJ1Driver <input> <output>");
         System.exit(1);
      }

//      THE_LOGGER.info("inputDir="+args[0]);
//      THE_LOGGER.info("outputDir="+args[1]);
      int returnStatus = ToolRunner.run(new AverageDriver(), args);
