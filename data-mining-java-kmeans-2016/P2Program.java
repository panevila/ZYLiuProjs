package fl16_cse514hw2p1;

import java.io.*;
import java.util.*;

public class P2Program {

	public static void main(String[] args) throws IOException {
		
		BufferedReader reader = new BufferedReader(new FileReader("./data/ad_reduced1.arff"));
		int nFeatures = 200;
		...
		for(; (tmpLine = reader.readLine()) != null; ){
			//jump to data
			if(isData == false){
				if(tmpLine.equals("@data")) isData = true;
				continue;
			}
			// store data
		...
		KMeanRetClass rs = basicKMeans(dataList, 3, 1000, 1, false, calcFunc);
		double[] eval01 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 0, 1, evalFunc);
		double[] eval02 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 0, 2, evalFunc);
//		double[] eval03 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 0, 3, evalFunc);
		double[] eval12 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 2, 1, evalFunc);
//		double[] eval13 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 1, 3, evalFunc);
//		double[] eval23 = evalClustering(rs.clusterDataList, rs.centroidList, rs.clusterAssign, 2, 3, evalFunc);
	...
	static KMeanRetClass basicKMeans(List<double[]> dataList, 
		...
		return new KMeanRetClass(clusterAssign, centroidList, clusterDataList);
	}
	...
	static class KMeanRetClass{
		...
		KMeanRetClass(int[] clusterAssign, List<double[]> centroidList, List<List<double[]>> clusterDataList){
