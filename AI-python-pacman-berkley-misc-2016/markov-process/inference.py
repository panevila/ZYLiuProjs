...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance==None:
        newBeliefs[self.getJailPosition()] == 1.0   #if in jail
    else:
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            probEmGivenDistance = emissionModel[trueDistance]   #P(e|x)
            if probEmGivenDistance > 0: 
                newBeliefs[p] = self.beliefs[p] * probEmGivenDistance   #P(x|e) prop P(x')*P(e|x)
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
      ...
      for newP, probNposGivenOpos in newPosDist.iteritems():
          newBeliefs[newP] = newBeliefs[newP] + self.beliefs[p]*probNposGivenOpos      #sum P(x|e)*P(x|x')
    self.beliefs = newBeliefs
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    pacmanPosition = gameState.getPacmanPosition()
    "*** YOUR CODE HERE ***"
    particleWeightSum = util.Counter()
    allZero = False
    if noisyDistance == None:
        particleWeightSum[self.getJailPosition()] = 1.0
    else:
        allZero = True
        for p in self.particles:
            ...
            if probEmGivenDistance != 0:
                allZero = False;
            particleWeightSum[p] = particleWeightSum[p] + probEmGivenDistance
        particleWeightSum.normalize()

    if noisyDistance != None and allZero == True:
        self.initializeUniformly(gameState)
    else:
        self.particles = list()
        for i in range(0,self.numParticles):
            self.particles.append(util.sample(particleWeightSum))
  ...
  def getBeliefDistribution(self):
    ...
    for p in self.legalPositions:
        distribution[p] = 0.0
    for p in self.particles:
        distribution[p] = distribution[p] + 1.0
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(0,self.numParticles):
        ...
        for i in range(0,self.numGhosts):
            tmpList.append(random.choice(self.legalPositions))  #select a place and put
        self.particles.append(tuple(tmpList))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
        ...
        for i in range(0,self.numGhosts):
            newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldPos),
                                                   i, self.ghostAgents[i])
  ...
  def observeState(self, gameState):
    ...
    for p in self.particles:
        ...
        for i in range(0,self.numGhosts):
            ...
            probEmGivenDistance = emissionModels[i][trueDistance]
            tmpJointProb = tmpJointProb * probEmGivenDistance       #calculate joint prob
        particleWeightSum[p] = particleWeightSum[p] + tmpJointProb
        if particleWeightSum[p] != 0:
            allZero = False;
    particleWeightSum.normalize()
    ...
    if allZero == True:
        ...
        for i in range(0,self.numParticles):
            if hasInJail == True:                 #if there are ghosts in jali, we need to modify this new tuple
                tmpList = list(util.sample(particleWeightSum))
                for j in range(0,self.numGhosts):
                    if noisyDistances[j] == None:
                        tmpList[j] = self.getJailPosition(j)
                newParticles.append(tuple(tmpList))
            else:
                newParticles.append(util.sample(particleWeightSum))
        self.particles = newParticles
