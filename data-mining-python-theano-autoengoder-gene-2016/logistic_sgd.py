...
def load_data(dataset):
    ...
    for i in range(182, 291):
        valid_x.append(data[i][0:8560])
        valid_y.append(data[i][8560])
    for i in range(291, 364):
        test_x.append(data[i][0:8560])
        test_y.append(data[i][8560])
    del data
    # Load CSV End
...
def sgd_optimization_mnist(learning_rate=0.03, n_epochs=1000,
                           dataset='./data_group/all_labeled_no_text.csv',
                           batch_size=20):
    """
    ...
    classifier = LogisticRegression(input=x, n_in=8560, n_out=2)
